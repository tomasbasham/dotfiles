...
-- ============================================================================
-- Plugin configuration for editor-related plugins.
...
return {
    ...
    config = function(_, opts)
      ...
      local function toggle_replace()
        ...
        if api.tree.is_visible() then
          ...
          api.tree.toggle({ current_window = true, find_file = true, path = vim.fn.expand('%:p:h') })
        end
      end
...

  {
    ...
    enabled = true,
    keys = {
      { '<C-n>', desc = 'Select next match', mode = { 'n', 'v' } },
    },
    init = function()
...

  {
    ...
    enabled = true,
    keys = {
      { '<C-p>', desc = 'Open files search' },
      { '<S-l>', desc = 'Open lines search' },
      { '<S-b>', desc = 'Open buffers search' },
      { '<S-h>', desc = 'Open history search' },
    },
    opts = {
      winopts = {
        preview = {
...

  {
    ...
    config = function()
      ...
      vim.keymap.set('n', '<Leader>a', ':Grep ', options)
      ...
      -- automatically.
      vim.keymap.set('v', 'gs', '<plug>(GrepperOperator)', options)
    end,
  },
