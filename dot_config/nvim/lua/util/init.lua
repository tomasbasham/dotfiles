...
-- ============================================================================
-- Utility functions for the TangoBravo Neovim configuration.
...
function M.default_node_binary()
  local node_versions = os.getenv('NODE_VERSIONS')
  local default_node_version = table.concat({ node_versions, 'default' }, '/')
  if vim.fn.isdirectory(default_node_version) == 1 then
    return table.concat({ default_node_version, 'bin', 'node' }, '/')
  end
  return nil
end
