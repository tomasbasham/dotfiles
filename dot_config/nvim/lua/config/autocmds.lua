...
autocmd('BufReadPost', {
  pattern = '*',
  callback = function()
    if vim.tbl_contains(ignore_filetypes, vim.opt_local.filetype:get()) then return end
    if vim.tbl_contains(ignore_buftypes, vim.opt_local.buftype:get()) then return end
